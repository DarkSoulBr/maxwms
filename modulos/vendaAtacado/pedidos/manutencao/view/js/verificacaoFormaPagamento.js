...
$(function()
{
	...
	var result = '0.000';
	var valorDuplicata = '0';
	

	function habilitaEdicaoDiasVencimento() {
	
		 $("th > div:contains('Dias'),.saltoDuplicata").click(function()
		//$("input[name='fecdia']").hover(function()
		//$("input[name='fecbanco']").hover(function()
			{
			...
			//var html = '<h5>OBS.: RESULTADO DETALHADO DO ERRO 424. ';
						...
						/*
						 ...
						 if($("#permAlterarSalto").val() !== '0') {
						 
						var senhaDigitada;
						var html = '';
   						html +='</table>'+
   						'\n <br /><br />'+
   						'\n DIGITE SUA SENHA PARA EDITAR A QTD DE DIAS PARA O VENCIMENTO: <input type="password" name="senhaAlteraSalto" id="senhaAlteraSalto" size"6"> ';
   						
   						
   						
   						$('#retornoLiberacao').attr('title','ATENCAO');
   						...
   						$('#retornoLiberacao').dialog(
   								{
									...
									senhaDigitada = $("#senhaAlteraSalto").val();
																		
									var usuarioPrefechamento = new Object();
									usuarioPrefechamento.usuario = usuario.codigo;
									usuarioPrefechamento.senha = usuario.senha;
									
									if (MD5(senhaDigitada) != usuarioPrefechamento.senha) {
										msg = "<br> - SENHA INCORRETA";
										alert('SENHA INCORRETA');
										$("#retorno").messageBox(msg,false, false);
										$("#btnLiberacaoPedido").attr('disabled', 'disabled');
										return false;
									} else {
										$("input[name='fecdia']").removeAttr('disabled');
										$("input[name*='txtVencimentoDuplicata']").removeAttr('disabled');
									}
											...
											$.post('modulos/vendaAtacado/pedidos/manutencao/controller/acoes.php',
   							   				...
   							   				function(data)
   							   				{
													...
													teste = ""+retorno.mensagem;
													

   							   					$(this).dialog("destroy");
													if ( retorno.mensagem.match(/OK/ig)){
													
														$("#alteraSalto").val(senhaDigitada);
														$("input[name='fecdia']").removeAttr('disabled');
														$("input[name*='txtVencimentoDuplicata']").removeAttr('disabled');
													} 
													else {
														alert(retorno.mensagem); //SENHA INCORRETA
													}
	...
	//pedidoValor = (pedido.pvvalor-pedido.pvvaldesc);
		...
		if((estoqueOrigem=="SP" && estoqueDestino=="BA") || (estoqueOrigem=="SP" && estoqueDestino=="MG") || (estoqueOrigem=="SP" && estoqueDestino=="RS")|| (estoqueOrigem=="SP" && estoqueDestino=="PE"))
		{
...
//											 }
								...
								//  if(value.fecforma!=105 )
									  ...
									  if(value.fecforma==105)
									  {
										  ...
										  if(value.fecforma == 101 )
										  {
																		  valPrefec++; // Contador para certificar a quantidade de parcelas de prefechamento existente, será utilizada para não gerar mais parcelas que a cond. comercial.
										  }
									 ...
									 // if ( possuiChequePre == true ) {
								  	...
								  	tblFormaPagamento['tabela'] +='</td><td><input type="text" name="fecdata"   size="12" id="fecdata'+value.prefeccodigo+'" value="'+value.fecdata+'" size="10"></td>';
									
									if(value.fecforma!=103)
									  {
									tblFormaPagamento['tabela'] += '<td><input type="text" disabled name="fecvecto"   size="12" id="fecvecto'+value.prefeccodigo+'" value="'+value.fecvecto+'" size="10"></td>';
									} else {
									tblFormaPagamento['tabela'] += '<td><input type="text"  name="fecvecto"   size="12" id="fecvecto'+value.prefeccodigo+'" value="'+value.fecvecto+'" size="10"></td>';
									}
									
									if(value.fecforma==101) {
									tblFormaPagamento['tabela'] += '<td><input type="text" name="fecdia" disabled size="3" maxlength="3"  id="fecdia'+value.prefeccodigo+'" value="'+value.fecdia+'" size="10"></td>';
									} else {
									tblFormaPagamento['tabela'] += '<td><input type="text" name="fecdia"  size="3" maxlength="3"  id="fecdia'+value.prefeccodigo+'" value="'+value.fecdia+'" size="10"></td>';
									}
									
									
									tblFormaPagamento['tabela'] +=  '<td><input type="text" name="fecbanco" size="3" maxlength="3"  id="fecbanco'+value.prefeccodigo+'" value="'+value.fecbanco+'" size="10"></td>'+
	...
	$("#listaParcelas").ingrid({
		...
		paging: false,
		sorting: false,
		colWidths:[180,190,80,90,90,40,45,60,60,130],
		height: 150
	});
	/*} else { 
	...
	$("#listaParcelas").ingrid({
		...
		paging: false,
		sorting: false,
		colWidths:[180,80,90,90,40,45,60,60,130],
		height: 150
	});
	}*/
 	...
 	$("input[name='btnExcluirParcela']").click(function()
			{
                                ...
                                if( (pedido.pvencer2 == 3 || pedido.pvencer2 ==4) && (usuario.nivel !=2 && usuario.codigo != 262 && usuario.codigo != 359)){

                                msg = "<br> - PEDIDO LIBERADO PARA O FINANCEIRO";
                                alert('PEDIDO LIBERADO PARA O FINANCEIRO - FAVOR ENTRAR EM CONTATO COM O FINANCEIRO');
                                $("#retorno").messageBox(msg,false, false);
                                $("#btnLiberacaoPedido").attr('disabled', 'disabled');
                                return false;
...
//                                 else if( (pedido.pvlibera!=null && pedido.pvhora!=null) && (usuario.codigo != 262 && usuario.codigo != 359)){
...
//                                 } 
                                        ...
                                        $.post('modulos/vendaAtacado/pedidos/manutencao/controller/acoes.php',
                                                        ...
                                                        function(data)
                                                        {
                                                                ...
                                                                diferenca = (diferenca.toFixed(2) == "-0.00")?"0.00":diferenca;
	...
	{
//tmpCondicaoComercial.parcela-valPrefec

//alert("no verificacaoFormaPagto");
...
//			$('#lblTotalLimiteDisponivel').text($.mask.string(parseFloat(limiteDisponivel).toFixed(2), 'decimal'));
			...
			//tblFormaPagamento['duplicata'] += '<br /><br /><table border="1" cellpadding="0" cellspacing="0" width="600">'+'<tr>'+'<td>Forma Pagto</td><td>Num. Doc.</td><td>Banco</td><td>	Agência</td><td>	Conta</td><td>Valor</td><td>Vencimento</td><td>Data Vencto</td>'+'</tr>';
			tblFormaPagamento['duplicata'] += '<br /><br /><table border="1" cellpadding="0" cellspacing="0" width="600">'+'<tr>'+'<td>Forma Pagto</td><td>Num. Doc.</td><td>Banco</td><td>	Agência</td><td>	Conta</td><td>Valor</td><td class="saltoDuplicata">Venc. em Dias</td></tr>';
			//informações da conta (Banco, Ag. C/C)
//GERA SOMENTE AS RESTANTES P COMPT O TOTAL PARCELAS
				//informações da conta (Banco, Ag. C/C)
                                
//				for (i = 1; i <= tmpCondicaoComercial.parcela; i++)
				for (i = 1,j=0; i <= (tmpCondicaoComercial.parcela - valPrefec ); i++)
                                                {
                                                        ...
                                                        {
//                                                                descricaoParcelas = tmpCondicaoComercial.descricao;
//                                                                saltoTmp = descricaoParcelas.split("/");// corrigir, nem sem terá barra. SE BASEAR NO SALTO, PARCELA E INICIAL
//                                                                salto = Number(saltoTmp[(i-1)+valPrefec]);
                                                                    salto = (((i-1)+valPrefec)*Number(tmpCondicaoComercial.salto))+Number(tmpCondicaoComercial.inicial);
...
//    +"\nsalto: "+salto
//    +"\nvenc: "+venc
//    +"\n tmpCondicaoComercial.descricao: "+tmpCondicaoComercial.descricao
//    +"\n dataVencimentoDuplicata: "+dataVencimentoDuplicata
//    +"\n txtdataVencimentoDuplicata: "+txtdataVencimentoDuplicata
//);
...
//                                                totalDiferenca = valParcela2.toFixed(2) * tmpCondicaoComercial.parcela;
                                                valParcela2 = diferenca/(tmpCondicaoComercial.parcela-valPrefec);
                                                totalDiferenca = valParcela2.toFixed(2) * (tmpCondicaoComercial.parcela-valPrefec);
...
/*
                                                tblFormaPagamento['duplicata'] += '<td><input type="text" name="txtVencimentoDuplicata'+i+'" id="txtVencimentoDuplicata'+i+'" value="'+ venc +'"  size="12"></td>'+'<td><input name="txtDataVencimentoDuplicata'+i+'" id="txtDataVencimentoDuplicata'+i+'" value="'+ txtdataVencimentoDuplicata +'" type="text" size="10"></td>'+'</tr>';*/
												
											//	}
												tblFormaPagamento['duplicata'] += '<td><input type="text" disabled name="txtVencimentoDuplicata'+i+'" id="txtVencimentoDuplicata'+i+'" value="'+ venc +'"  size="12"></td></tr>';
...
j++;
			...
			//tblFormaPagamento['cartao'] += '<br /><br /><table border="1" cellpadding="0" cellspacing="0" width="600">'+'<tr>'+'<td>Forma Pagto</td><td>Num. Doc.</td><td>Valor</td><td>Vencimento</td><td>Data Vencto</td>'+'</tr>';
			tblFormaPagamento['cartao'] += '<br /><br /><table border="1" cellpadding="0" cellspacing="0" width="600">'+'<tr>'+'<td>Forma Pagto</td><td>Num. Doc.</td><td>Valor</td><td>Venc. em Dias</td></tr>';
...
//                                                                                                                salto = ( valPrefec + i ) * Number(tmpCondicaoComercial.salto)+salto;
//                                                                                                                salto += Number(tmpCondicaoComercial.salto);
                                                                                                                    descricaoParcelas = tmpCondicaoComercial.descricao;
                                                                                                                    saltoTmp = descricaoParcelas.split("/");
                                                                                                                    salto = Number($.trim(saltoTmp[(i-1)+valPrefec]));
...
//                                tblFormaPagamento['cartao'] += $('#listaBandeirasCartoesBase').html();
			...
			//tblFormaPagamento['cartao'] += '<td><input type="text" name="txtVencimentoCartao'+i+'" id="txtDiaVencimentoCartao'+i+'" value="'+ venc +'" size="10"> </td>'+ 												'<td><input name="txtdataVencimentoCartao'+i+'" id="txtdataVencimentoCartao'+i+'" value="'+ txtdataVencimentoCartao +'" type="text" size="10"></td>'+'</tr>';
			tblFormaPagamento['cartao'] += '<td><input type="text" name="txtVencimentoCartao'+i+'" id="txtDiaVencimentoCartao'+i+'" value="'+ venc +'" size="10"> </td></tr>';
												 }
			...
			//tblFormaPagamento['ordem'] += '<table border="1" cellpadding="0" cellspacing="0" width="600"><tr><td>Forma Pagto</td><td>Num. Doc.</td><td>Banco</td><td>	Agência</td><td>	Conta</td><td>Valor</td><td>Vencimento</td><td>Data Vencto</td></tr>';
			tblFormaPagamento['ordem'] += '<table border="1" cellpadding="0" cellspacing="0" width="600"><tr><td>Forma Pagto</td><td>Num. Doc.</td><td>Banco</td><td>	Agência</td><td>	Conta</td><td>Valor</td><td>Vencimento</td></tr>';
													...
													{
//                                                                                                        salto = salto + Number(tmpCondicaoComercial.salto);
//                                                                                                        salto =( valPrefec + i ) * Number(tmpCondicaoComercial.salto);
//                                                                                                        salto += (Number(tmpCondicaoComercial.salto));
                                                                                                             descricaoParcelas = tmpCondicaoComercial.descricao;
                                                                                                             saltoTmp = descricaoParcelas.split("/");
                                                                                                             salto = Number($.trim(saltoTmp[(i-1)+valPrefec]));
...
//			totalDiferenca = valParcela2.toFixed(2) * tmpCondicaoComercial.parcela;
			...
			result = Math.round(tt*100)/100;
			  ...
			  tblFormaPagamento['conta']+= '<hr size="1"><h4>Folha '+i+'</h4><table border="0" cellpadding="0" cellspacing="0"><tr> <td width="170" height="25">Numero Cheque:</td> <td width="140" colspan="5"><input name="txtNumeroDocFolha'+i+'" id="txtNumeroDocFolha'+i+'"  type="text" size="17"> Data:<input name="txtDataFolha'+i+'" id="txtDataFolha" value="'+ dataVencimentoChequesPre +'" type="text" size="10">'
			//tblFormaPagamento['conta']+= 'Salto:<input name="txtSaltoFolha'+i+'" id="txtSaltoFolha'+i+'" value="'+ venc +'"type="text" size="3"></td> ';
			tblFormaPagamento['conta']+= 'Salto:<input name="txtSaltoFolha'+i+'" id="txtSaltoFolha'+i+'" value="'+ 0 +'"type="text" size="3"></td> ';
...
//					valParcela = (diferenca/tmpCondicaoComercial.parcela);
					valParcela = (diferenca/(tmpCondicaoComercial.parcela-valPrefec));
...
//				$('#lblTotalLimiteDisponivel').val($.mask.string(parseFloat(pedido.cliente.clilimite).toFixed(2), 'decimal'));
		...
		$("#lblTotalPago").text(pedido.TotalValorPreFechamento);
		diferenca = pedidoValor-pedido.TotalValorPreFechamento;
		diferenca = (diferenca == "0" || diferenca.toFixed(2) == "-0.00" )?"0.00":diferenca;
...
//                                                                                                    $("#btnIncluirFormaPagamento").removeAttr('disabled');

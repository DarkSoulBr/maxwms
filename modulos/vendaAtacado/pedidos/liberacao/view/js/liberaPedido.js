 ...
 * @author Douglas
 *
 * CRUD - a√ß√µes jquery para update para liberar pedidos.
 *
 * Cria√ß√£o    05/02/2010
    ...
    $("#btnLiberacaoPedido").click(function()
    {
        ...
        if($('#formLiberacaoPedido').validate().form())
        {
            ...
            if(pedido.tipoPedido.sigla=='R'){
                ...
                msg = "<br> - PEDIDO RESERVA N√O PODE SER LIBERADO";
                alert('PEDIDO RESERVA N√O PODE SER LIBERADO');
            ...
            valorPreFechamentoAtual = 0;
            $.each(pedido.preFechamento, function (key, valueX) {
                if (valueX.fecforma != '100' && valueX.fecforma != '102' && valueX.fecforma != '106' && valueX.fecforma != '105' && valueX.fecforma != '111'  && valueX.fecforma != '112' ) {
                    valorPreFechamentoAtual += Number(valueX.fecvalor);
                }
            });
       ...
       //        if(pviest09 != 0)
        ...
        //            if(pviest09 != pvisaldo)
            ...
            dataValidacao = new Date().date(pedido.cliente.clidatavalida);
            dataSerasa = new Date().date(pedido.cliente.cliserasa);
            dataSintegra = "";
            dataReceita = "";
            ...
            var clienteIsento = false;

             if  (!cliie.match(/[0-9]/i)) {
                               clienteIsento = true;
                           }

             if( pedido.cliente.clireceitadataultimaconsulta != undefined
                                &&  pedido.cliente.clireceitadataultimaconsulta != null ) {
                                dataReceita = pedido.cliente.clireceitadataultimaconsulta;
                            ...
                            if( pedido.cliente.clisintegradataultimaconsulta != undefined
                                &&  pedido.cliente.clisintegradataultimaconsulta != null
                                &&  cliie.match(/[0-9]/i)) {
                                dataSintegra = pedido.cliente.clisintegradataultimaconsulta;
                                    ...
                                    if(pedido.tipoPedido.sigla!='X')
                                    {
                                        if(pedido.tipoPedido.sigla!='EP')
                                        {
                                            ...
                                            if(pedido.cliente.clipessoa == '1') {
                                                        ...
                                                        }else if ( !pedido.cliente.clireceitasituacaocadastral.match(/ativ/i)) {
                                                            msg += "<br> - CLIENTE IRREGULAR NA RECEITA";
                                                            alert('CLIENTE IRREGULAR NA RECEITA ');
                                                            $("#retorno").messageBox(msg,false, false);
                                                            //$("#btnLiberacaoPedido").attr('disabled', '');
                                                            return false;
                                                        }else if ( (pedido.cliente.clisintegrasituacaocadastral == undefined || pedido.cliente.clisintegrasituacaocadastral == "" || pedido.cliente.clisintegrasituacaocadastral == null ||
                                                        pedido.cliente.clisintegrasituacaocadastral.match(/CANCEL|NAO|BAIXADO|FALH|INAB|IRREG|INAT|INVA|0/i)) && clienteIsento == false) {
                                                        ...
                                                        if(pedido.cliente.enderecoFaturamento.cidade.descricao != pedido.cliente.enderecoFaturamento.cidadeIbge.descricao)
                                                        {
                                                            msg = "<br> - CIDADE IBGE DIFERENTE DA CIDADE CADASTRADA NO ENDERECO FATURAMENTO";
                                                            $("#retorno").messageBox(msg,false, false);
                                                            msgPreFec =" CIDADE IBGE DIFERENTE DA CIDADE CADASTRADA NO ENDERECO FATURAMENTO";
                                                            return false;
                                                        }


                                                        if(pedido.cliente.clidatavalida == null || pedido.cliente.clidatavalida == undefined || pedido.cliente.clidatavalida == '' || pedido.cliente.clidatavalida == '0')
                                                        {
                                                            msg = "<br> - DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - FAVOR ENCAMINHAR AO FINANCEIRO";
                                                            //					msg += "<br> - DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - SER√? ENCAMINHADO AO FINANCEIRO";
                                                            //					alert('DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - SER√? ENCAMINHADO AO FINANCEIRO');
                                                            $("#retorno").messageBox(msg,false, false);
                                                            msgPreFec ="DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - FAVOR ENCAMINHAR AO FINANCEIRO";
                                                            //					msgPreFec +="DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - SER√? ENCAMINHADO AO FINANCEIRO";
                                                            return false;
                                                        }

//                                                     if(pedido.cliente.clidatavalida <  dataLimite)
                                                        if(dataValidacao <  dataLimiteCad)
                                                        {
                                                            msg = "<br> - DADOS CADASTRAIS DO CLIENTE ENCONTRAM-SE DESATUALIZADOS - FAVOR ENCAMINHAR AO FINANCEIRO";
                                                            //					msg += "<br> - DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - SER√? ENCAMINHADO AO FINANCEIRO";
                                                            //					alert('DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - SER√? ENCAMINHADO AO FINANCEIRO');
                                                            $("#retorno").messageBox(msg,false, false);
                                                            msgPreFec ="DADOS CADASTRAIS DO CLIENTE ENCONTRAM-SE DESATUALIZADOS -  FAVOR ENCAMINHAR AO FINANCEIRO";
                                                            //					msgPreFec +="DADOS CADASTRAIS DO CLIENTE N√O ENCONTRAM-SE VALIDADOS - SER√? ENCAMINHADO AO FINANCEIRO";
                                                            return false;
                                                        }
                                                    }
                                            ...
                                            if (pedido.cliente.clilimite !== undefined && pedido.cliente.clilimite !== null && pedido.cliente.clilimite !== '' && pedido.cliente.clilimite !== '0') {
                                                //                           limiteDisponivel = Number(pedido.cliente.clilimite) - Number(pedido.cliente.vendasEmAberto.totalVendas)-Number(valorPreFechamentoAtual);
                                                limiteDisponivel = Number(pedido.cliente.clilimite) - Number(pedido.cliente.vendasEmAberto.cobrancaAberto)-Number(valorPreFechamentoAtual);
                                                //                            limiteMostrar = (Number(pedido.cliente.clilimite) - Number(pedido.cliente.vendasEmAberto.totalVendas)).toFixed(2);
                                                limiteMostrar = (Number(pedido.cliente.clilimite) - Number(pedido.cliente.vendasEmAberto.cobrancaAberto)).toFixed(2);
                                            }
                                        ...
                                        if(pedido.tipoPedido.sigla!='R')
					{
                                            if(pedido.tipoPedido.sigla!='I')
                                            {
                                                ...
                                                $.each(pedido.preFechamento, function (key, value)
                                                {
                                                    ...
                                                    if(value.fecforma !='100' && value.fecforma !='106' && value.fecforma !='111' && value.fecforma !='112')
                                                    {
								...
								{
//                                                                    if(pedido.cliente.cliserasaexp < dataHoje)
                                                                    if(dataSerasa < dataLimiteSer)
                                                                    {
/*alert( "dataSerasa -> "+dataSerasa +"\n dataLimiteSer -> "+dataLimiteSer
+"\n limitePrazo -> "+limitePrazo
+"\n prazolimiteSer -> "+prazolimiteSer

);*/
                                                                        msg += "SERASA EXPIRADO, CONTATE O FINANCEIRO";
                                                                        alert('SERASA EXPIRADO, CONTATE O FINANCEIRO ');
                                                                        $("#retorno").messageBox(msg,false, false);
                                                                        $("#btnLiberacaoPedido").attr('disabled', '');
                                                                        msgPreFec +="SERASA EXPIRADO, CONTATE O FINANCEIRO";
                                                                        return false;
								    }
								}
                                                            }
                                                    ...
                                                    if((limiteDisponivel < 0) && value.fecforma=='101')
                                                    {
                                                        ...
                                                        $("#retorno").messageBox(msg,false, false);
                                                        msgPreFec ="FORMA DE PAGAMENTO SOMENTE PODE SER A VISTA OU COM CARTAO OU ORDEM PAGTO";
                                                        //				msgPreFec +="FORMA SOMENTE PODE SER A VISTA OU COM CARTAO OU ORDEM PAGTO";
                                                        return false;
                                                    }

                                                    if((limiteDisponivel < 0) && (value.fecforma=='103' || value.fecforma=='110'))
                                                    {
                                                        if(limiteDisponivel < 0 ||limiteMostrar <= 0) {
                                                            msg = "<br /> - FORMA DE PAGAMENTO SOMENTE PODE SER A VISTA OU COM CARTAO OU ORDEM PAGTO. NAO ACEITA CHEQUE";
                                                        } else {
                                                            msg = "<br /> - DISPONIVEL O LIMITE DE "+ limiteMostrar +" PARA PAGAMENTO EM CHEQUE. O RESTANTE PODE SER PAGO A VISTA OU COM CARTAO.";
                                                        }
                                        ...
                                        if(pedido.tipoPedido.sigla!='R')
                                        {
                                            if(pedido.tipoPedido.sigla!='I')
                                            {
                                                if((limiteDisponivel < 0) && pedido.preFechamento.fecforma=='101')
                                                {
                                                    if(limiteDisponivel < 0 ||limiteMostrar < 0) {
                                                        msg = "<br /> - FORMA DE PAGAMENTO A VISTA OU COM CARTAO OU ORDEM PAGTO. LIMITE ZERO. NAO ACEITA DUPLICATA";
                                                    } else {
                                                        msg = "<br /> - DISPON√?VEL O LIMITE DE "+ limiteMostrar +" PARA PAGAMENTO EM DUPLICATA. O RESTANTE PODE SER PAGO A VISTA OU COM CARTAO.";
                                                    }
                                                    $("#retorno").messageBox(msg,false, false);
                                                    $("#btnLiberacaoPedido").attr('disabled', '');
                                                    return false;
                                                ...
                                                if((limiteDisponivel < 0) && (pedido.preFechamento.fecforma=='103' || pedido.preFechamento.fecforma=='110'))
                                                {
                                                    if(limiteDisponivel < 0 ||limiteMostrar < 0) {
                                                        msg = "<br /> - FORMA DE PAGAMENTO SOMENTE PODE SER A VISTA OU COM CARTAO OU ORDEM PAGTO. NAO ACEITA CHEQUE";
                                                    } else {
                                                        msg = "<br /> - DISPON√?VEL O LIMITE DE "+ limiteMostrar +" PARA PAGAMENTO EM CHEQUE. O RESTANTE PODE SER PAGO A VISTA OU COM CARTAO.";
                                                    }
                                                    $("#retorno").messageBox(msg,false, false);
                                                    $("#btnLiberacaoPedido").attr('disabled', '');
                                                    return false;
...
//            pedido2.pvlibdep = 1;
        ...
        function(data)
        {
            ...
            else
            {
                ...
                var codRecebido=new RegExp(0);
                var libdep1=new RegExp(1);

                if(data.codigo!='424' )
                {


//									if(data.codigo=== '0' && pedido2.pvlibdep === '1') {
									if(codRecebido.test(data.codigo) && (libdep1.test(pedido2.pvlibdep) || libdep1.test(pedido2.pvlibvit))) {
					//alert("chamando exportawmspedidos.php?pedido="+pedido2.pvnumero);

											$.get('exportawmspedidos.php',
												{
													pedido:pedido2.pvnumero
					//                                pedido:JSON.stringify(pedido2.pvnumero)
...
//											if(pedido2.pvlibdep === 1) {
											if(libdep1.test(pedido2.pvlibdep) || libdep1.test(pedido2.pvlibvit)) {
													$.get('exportawmspedidos.php',
													{
														pedido:pedido2.pvnumero
						//                                pedido:JSON.stringify(pedido2.pvnumero)

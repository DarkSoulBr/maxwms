...
$sql = "SELECT max(a.clicod) as codigo FROM empresa a";
$sql = pg_query($sql);
$row = pg_num_rows($sql);
if ($row) {
...
$cadastro->insere("empresa", "(clinguerra,clicod,clirazao,clisuframa,clicnpj,cliie,clirg,clicpf,cliobs,clipessoa,cliinc,clicep,cliendereco,clibairro,clifone,clicomplemento,cliemail,cidcodigo,clinumero,cidcodigoibge)", "('$fornguerra','$forcod','$forrazao','$forsuframa','$forcnpj','$forie','$forrg','$forcpf','$forobs','$forpessoa','$atual','$fnecep','$fneendereco','$fnebairro','$fnefone','$fnecomplemento','$fneemail','$fnecodcidade','$fnenumero','$cidadeibge')");

$j = 0;
for ($i = 0; $i < 2; $i++) {
    $sql = "
    SELECT empcodigo From empresa 
    Where clicod = $forcod";
...
$forcodigo = pg_fetch_result($sql, "0", "empcodigo");

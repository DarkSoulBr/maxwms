...
require_once("include/relatorioresinventario.php");
...
if ($contagrupo > 0) {
    for ($i = 0; $i < $contagrupo; $i++) {
        $query_produto = $cadastro->seleciona("p.procod as codigo, p.prnome as nome,g.grunome as grupo,i.invantigo as antigo,i.invatual as atual, p.proref as referencia, p.proemb as embalagem, p.proreal as custo", "produto p left join grupo g on p.grucodigo=g.grucodigo left join inventario i on p.procodigo=i.procodigo and i.estcodigo=$estoque", "p.grucodigo=" . $grupox[$i] . " and p.procodigo=i.procodigo and i.invdata='$invdata'");
        ...
        if ($row > 0) {
            for ($j = 0; $j < $row; $j++) {
                ...
                if ($j == 0) {
                    ...
                    $pdf->Cell(9, 5, 'Código', 1, 0, 'C');
                    $pdf->Cell(80, 5, 'Produto', 1, 0);
                    $pdf->Cell(21, 5, 'Ref. Fornededor', 1, 0, 'C');
                    $pdf->Cell(13, 5, 'Embalagem', 1, 0, 'C');
                    $pdf->Cell(12, 5, 'Processado', 1, 0, 'C');
                    $pdf->Cell(10, 5, 'Digitado', 1, 0, 'C');
                    $pdf->Cell(7, 5, 'Falta', 1, 0, 'C');
                    $pdf->Cell(7, 5, 'Sobra', 1, 0, 'C');
                    $pdf->Cell(13, 5, 'Custo', 1, 0, 'C');
                    $pdf->Cell(13, 5, 'Total', 1, 1, 'C');
                }
                ...
                $aux = $array_produto->atual - $array_produto->antigo;
                if ($array_produto->antigo == "0" && $array_produto->atual == "0") {
                    $falta = 0;
                    $sobra = 0;
                } else
                if ($aux >= 0) {
                ...
                $total = $array_produto->custo * $array_produto->atual;

                $pdf->Cell(9, 5, '' . $array_produto->codigo, 1, 0, 'C');
                $pdf->Cell(80, 5, '' . $array_produto->nome, 1, 0);
                $pdf->Cell(21, 5, '' . $array_produto->referencia, 1, 0, 'L');
                $pdf->Cell(13, 5, '' . $array_produto->embalagem, 1, 0, 'C');
                $pdf->Cell(12, 5, '' . $array_produto->antigo, 1, 0, 'R');
                $pdf->Cell(10, 5, '' . $array_produto->atual, 1, 0, 'R');
                $pdf->Cell(7, 5, '' . $falta, 1, 0, 'R');
                $pdf->Cell(7, 5, '' . $sobra, 1, 0, 'R');
                $pdf->Cell(13, 5, '' . number_format($array_produto->custo, 2, ',', ''), 1, 0, 'R');
...
if ($contasubgrupo > 0) {
    for ($i = 0; $i < $contasubgrupo; $i++) {
        $query_produto = $cadastro->seleciona("p.procod as codigo, p.prnome as nome,s.subnome as subgrupo,i.invantigo as antigo,i.invatual as atual, p.proref as referencia, p.proemb as embalagem, p.proreal as custo", "produto p left join subgrupo s on p.subcodigo=s.subcodigo left join inventario i on p.procodigo=i.procodigo and i.estcodigo=$estoque", "p.subcodigo=" . $subgrupox[$i] . " and p.procodigo=i.procodigo and i.invdata='$invdata'");
        $row = pg_num_rows($query_produto);
...
if ($fornecedor != "") {    
    ...
    $query_produto = $cadastro->seleciona("p.procod as codigo, p.prnome as nome,f.fornguerra as fornecedor,i.invantigo as antigo,i.invatual as atual, p.proref as referencia, p.proemb as embalagem, p.proreal as custo", "produto p left join fornecedor f on p.forcodigo=f.forcodigo left join inventario i on p.procodigo=i.procodigo and i.estcodigo=$estoque", "f.forcod=" . $fornecedor . " and p.procodigo=i.procodigo and i.invdata='$invdata'");
    $row = pg_num_rows($query_produto);

...
$codcoletor = trim($_GET["coletor"]);
$codsetor = trim($_GET["setor"]);

$aux = explode("/", $invdata);
$invdata = $aux[2] . '-' . $aux[1] . '-' . $aux[0] . ' 00:00:00-03';

header("Content-type: " . "application/vnd.ms-excel");
header("Content-disposition: attachment; filename=relatoriocontagemsetor.xls");
header("Expires: 0");
...
$query = $cadastro->seleciona("setor", "setor", "setcodigo=$codsetor");
$array = pg_fetch_object($query);

$estoque2 = $array->setor;
...
//TITULO
echo "<td bgcolor='#CCCCCC' width='820' class='borda' colspan='12' align='center' valign='middle' height='35'><font color='#000000'><b>Relatório Gerencial de Contagem de Estoque</b>";
echo "<br>Contagem:</b> " . $estoque1 . " - Setor: " . $estoque2 . "</font></td></tr>";
...
if ($row > 0) {
    ...
    //for($j=0;$j<100;$j++)
        ...
        if ($codcoletor == '0' || $codcoletor == 0) {
            $sql2 = "
				SELECT a.colnome,sum(b.quantidade) as estoque 
				FROM coletores a,inventariocontagemsetorproduto b where a.colcodigo = b.colcodigo and b.procodigo = " . $array_produto->procodigo . " and b.invdata = '$invdata' and b.concodigo = $contagem and b.setcodigo = $codsetor group by a.colnome,a.colcodigo order by a.colcodigo";
        } else {
            $sql2 = "
				SELECT a.colnome,b.quantidade as estoque 
				FROM coletores a,inventariocontagemsetorproduto b where a.colcodigo = b.colcodigo and b.procodigo = " . $array_produto->procodigo . " and b.invdata = '$invdata' and b.concodigo = $contagem and setcodigo = $codsetor and b.colcodigo = $codcoletor order by a.colcodigo";
        }
        ...
        if ($row2) {
            ...
            for ($j2 = 0; $j2 < $row2; $j2++) {
                $coletorx = pg_fetch_result($sql2, $j2, "colnome");
                ...
                if ($quantidade > 0) {
                    ...
                    echo "<td width=\"120\" align=\"left\" class='borda' bgcolor='#FFFFFF' bordercolor='#3366CC'><font color='#000000'><b>" . $coletorx . "</b></font></td>";

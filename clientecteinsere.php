...
$sql = "SELECT max(a.clicod) as codigo FROM clientescte a";
$sql = pg_query($sql);
$row = pg_num_rows($sql);
if ($row) {
...
$cadastro->insere("clientescte", "(clinguerra,clicod,clirazao,clisuframa,clicnpj,cliie,clirg,clicpf,cliobs,clipessoa,cliinc,clicep,cliendereco,clibairro,clifone,clicomplemento,cliemail,cidcodigo,clinumero,cidcodigoibge,clinfe)", "('$fornguerra','$forcod','$forrazao','$forsuframa','$forcnpj','$forie','$forrg','$forcpf','$forobs','$forpessoa','$atual','$fnecep','$fneendereco','$fnebairro','$fnefone','$fnecomplemento','$fneemail','$fnecodcidade','$fnenumero','$cidadeibge','$fornfe')");

$j = 0;
for ($i = 0; $i < 2; $i++) {
    $sql = "
    SELECT clicodigo From clientescte 
    Where clicod = $forcod";

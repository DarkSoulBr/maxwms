...
//Processa todos os arquivos que estão na Pasta jogando num vetor multidimensional o nome do arquivo e numero pedido.
$Notas = [];
...
foreach ($Notas as $Nfe):
    ...
    //Verifica se é um Pedido Mãe ou um Pedido AGRUPADO
    $mae = 0;
    ...
    //Fica em Looping na Tabela de Agrupamento para saber se todos que devem ser agrupados estão no Vetor
    if ($mae > 0) {
...
if ($pedido_mae > 0) {
                    ...
                    if ($row) {
                        ...
                        if ($nfetramodFrete != 0) {
                            ...
                            if ($deveprosseguir == 0) {
                                ...
                                $Chaves[] = ["chave" => $chavenfe, "nfe" => $numeroEmail];
    ...
    foreach ($Chaves as $AuxChave):
        extract($AuxChave);
        $cadastro->insere("ctenfechave", "(ctenumero,ctenfenome)", "('$cte_max','$chave')");
    endforeach;
...
echo "<meta HTTP-EQUIV='Refresh' CONTENT='2;URL=importageranfexmlagr.php?parametro=1'>";
exit;

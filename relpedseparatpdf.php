...
//Page header
    function Header() {
        ...
        $this->Cell(80, 5, '', 0, 0);
        $this->Cell(20, 5, 'PEDIDO DE VENDA', 0, 0);
        $this->Cell(90, 5, $dia . '/' . $mes . '/' . $ano . ' ' . $hor . ':' . $min, 0, 1, 'R');

        $Var1 = $this->_Xped();
        if ($Var1 > 0) {
            $Var1 = $this->_Xped();
            $this->Cell(1, 5, 'Pedido: ' . $Var1, 0, 0);
        }
...
for ($wn = 0; $wn < sizeof($c1); $wn++) {

    $pedido = $c1[$wn];

    $sql1 = "Update pvenda set pvimpresso = 1 Where pvnumero = '$pedido'";
    pg_query($sql1);

    $pvvia = 0;
    $sql1 = "Select pvvia from pvenda Where pvnumero = '$pedido'";
    $cad = pg_query($sql1);
    if ($cad) {
    ...
    $sql1 = "Update pvenda set pvvia = $pvvia Where pvnumero = '$pedido'";
    pg_query($sql1);

//$nome = "/home/delta/im".$pedido.".txt";
    /*
      $nome = "im".$pedido.".txt";
      if (file_exists($nome)) {
      unlink($nome);
      }
      $arquivo2 = fopen("$nome","x+");
      $gravar = $pedido;
      $gravar=substr($gravar.'        ',0,8);
      fputs($arquivo2, "$gravar\r\n");
      fclose($arquivo2);
     */
...
//	$pedido = trim($_GET["pvnumero"]);
    ...
    if ($pvtra != 0) {
        ...
        $pegar3 = "SELECT a.traendereco,a.tracep,a.tranguerra,a.traemail, b.descricao,b.uf FROM transportador a	 
		  LEFT JOIN cidades as b on a.cidcodigo = b.cidcodigo		     
        ...
        if ($row3) {
            ...
            $traemail = trim(pg_fetch_result($cad3, 0, "traemail"));
        }
    }
    ...
    if ($pvtipo == 'S') {
        ...
        if ($pvvinculo <> 0) {
            ...
            $pdf->Cell(38, 5, 'PEDIDO M홂 : ' . $pvvinculo, 0, 1, 'R');
        } else {
    ...
    $pdf->Cell(112, 5, 'CONDI합ES COMERCIAIS : ' . $pvcond, 0, 1);
    $pdf->Cell(112, 5, 'LOCAL DE ENTREGA : ' . $pvlocal, 0, 1);
    $pdf->Cell(112, 5, 'OBSERVA합ES : ' . $pvobs, 0, 1);
    $pdf->Cell(120, 5, 'TRANSPORTADORA : ' . $tranome, 0, 1);
    $pdf->Cell(120, 5, 'EMAIL DA TRANSPORTADORA : ' . $traemail, 0, 1);
...
//$pedido = trim($_GET["pvnumero"]);
    ...
    $pegar = "SELECT a.pviest09,a.pviest026,b.procod,b.prnome,b.proemb,b.proref,c.fornguerra,d.medsigla,e.endlocal,e.endlocal2
		  ,b.proaltcx,b.prolarcx,b.procomcx,b.propescx  
    ...
    $pdf->Cell(10, 5, '', 1, 0);
    $pdf->Cell(68, 5, 'SEPARA플O DE VOLUMES', 1, 0);
    $pdf->Cell(18, 5, '', 1, 0);
    $pdf->Cell(26, 5, '', 1, 0);
    $pdf->Cell(10, 5, '', 1, 0, 'R');
    $pdf->Cell(10, 5, '', 1, 0);
    ...
    $pdf->Cell(10, 5, '', 1, 0);
    $pdf->Cell(68, 5, 'SEPARA플O DE PICKING', 1, 0);
    $pdf->Cell(18, 5, '', 1, 0);
    $pdf->Cell(26, 5, '', 1, 0);
    $pdf->Cell(10, 5, '', 1, 0, 'R');
    $pdf->Cell(10, 5, '', 1, 0);
    ...
    $pegar = "SELECT a.pviest09,a.pviest026,b.procod,b.prnome,b.proemb,b.proref,c.fornguerra,d.medsigla,e.endlocal,e.endlocal2
          ,b.proaltura,b.prolargura,b.procompr,b.propeso
		  FROM pvitem a,fornecedor c,medidas d,produto b
		  LEFT JOIN endereco as e on b.procodigo = e.procodigo and e.etqcodigo = 9
          Where a.pvnumero = '$pedido'
		  And a.procodigo=b.procodigo
    ...
    $pegar = "SELECT a.pviindustria,b.procod,b.prnome,b.proemb,b.proref,c.fornguerra,d.medsigla,e.endlocal,e.endlocal2
		  ,b.proaltcx,b.prolarcx,b.procomcx,b.propescx  
		  FROM pvitem a,fornecedor c,medidas d,produto b
		  LEFT JOIN endereco as e on b.procodigo = e.procodigo and e.etqcodigo = 9
          Where a.pvnumero = '$pedido'
    ...
    for ($i = 0; $i < $row; $i++) {
        ...
        $campo1 = pg_fetch_result($cad, $i, "pviindustria");
        $procod = pg_fetch_result($cad, $i, "procod");
        $prnome = pg_fetch_result($cad, $i, "prnome");
        $prref = pg_fetch_result($cad, $i, "proref");
        $premb = pg_fetch_result($cad, $i, "proemb");
...
//$pedido = trim($_GET["pvnumero"]);
    ...
    if ($ver > 0) {
        ...
        //Looping para imprimir os Volumes
        $pdf->Cell(10, 5, '', 1, 0);
        $pdf->Cell(68, 5, 'SEPARA플O DE VOLUMES - DAI', 1, 0);
        $pdf->Cell(18, 5, '', 1, 0);
        $pdf->Cell(26, 5, '', 1, 0);
        $pdf->Cell(10, 5, '', 1, 0, 'R');
        $pdf->Cell(10, 5, '', 1, 0);
    ...
    if ($ver > 0) {
        ...
        $pdf->Cell(10, 5, '', 1, 0);
        $pdf->Cell(68, 5, 'SEPARA플O DE PICKING - DAI', 1, 0);
        $pdf->Cell(18, 5, '', 1, 0);
        $pdf->Cell(26, 5, '', 1, 0);
        $pdf->Cell(10, 5, '', 1, 0, 'R');
        $pdf->Cell(10, 5, '', 1, 0);

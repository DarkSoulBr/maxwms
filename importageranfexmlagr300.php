...
echo "<h3>03/04 - Importacao de NF-e Agrupado (Alfaness)</h3>";
...
require_once("include/bancoerro.php");
require('code128.php');
...
if($pedido_mae>0){
	...
	foreach ($Notas as $Nfe):
		...
		//echo "Pedido: {$pedido} Mae: {$eh_mae} Filho: {$eh_filhote}<hr>";
	...
	// Essa situaacao acontece quando o usuario ja inclui o Ct-e individual para todas as notas do Agrupamento
	if($cteemitenteibge==null || $cteemitenteibge=='' || $eminumero==null || $eminumero==''){
		...
		if (trim($ctecidinicio)==""){$ctecidinicio = "0";} 
		if (trim($cteufinicio)==""){$cteufinicio = "0";} 
		if (trim($ctecidfinal)==""){$ctecidfinal = "0";} 
		if (trim($cteuffinal)==""){$cteuffinal = "0";} 
		...
		if (trim($percicm)==""){$percicm = "0";} 
		if (trim($percint)==""){$percint = "0";} 
		if (trim($percpob)==""){$percpob = "0";} 
		...
		if (trim($remcodigo)==""){$remcodigo = "0";} 							
		if (trim($cteremcidcodigo)==""){$cteremcidcodigo = "0";} 
		if (trim($cteremcidcodigoibge)==""){$cteremcidcodigoibge = "0";} 

						
		if(trim($ctedesnumero)=="" || !is_numeric(trim($ctedesnumero))){
			$ctedesnumero 		= 0;
		}
		if (trim($descodigo)==""){$descodigo = "0";} 								
		if (trim($ctedescidcodigo)==""){$ctedescidcodigo = "0";} 
		if (trim($ctedescidcodigoibge)==""){$ctedescidcodigoibge = "0";} 

		if(trim($cterecnumero)=="" || !is_numeric(trim($cterecnumero))){
			$cterecnumero 		= 0;
		}								
		if (trim($reccodigo)==""){$reccodigo = "0";}								
		if (trim($ctereccidcodigo)==""){$ctereccidcodigo = "0";} 
		if (trim($ctereccidcodigoibge)==""){$ctereccidcodigoibge = "0";} 
		...
		if (trim($ctecargaval)==""){$ctecargaval = "0";} 
		if (trim($cterntrc)==""){$cterntrc = "0";} 
		...
		//Gera o XML do CTe
		geraCteXml($cte_max,$chavenfemae,$Chaves,$emiteEmail);
...
echo "<meta HTTP-EQUIV='Refresh' CONTENT='2;URL=importageranfexmlagr300.php?parametro=1'>";
exit;	
...
function geraCteXml($parametro,$chavenfe,$Chaves,$emiteEmail){

...
function imprime_grupo(request)
{
	var xmldoc=request.responseXML;
   	...
   	var contador = '0';
	var tabela="<select id=\"grupo\" name=\"grupo\" multiple style=\"height:150px;width:300px;\" onkeypress=\"insertBeforeSelected('grupo',event,'grupo_selecionados')\">";
...
function imprime_fornecedor(request){
    ...
    var contador = '0';
	var tabela='<select id="pesquisa" name="pesquisa" multiple style="height:150px;width:290px;" onkeypress="insertBeforeSelected(pesquisa,event,\'fornecedor_selecionados\');">';
...
function imprelatorioexexcel(){
	...
	if(invdata==''){
		...
		if (document.getElementById("radio1").checked==true){
			window.open('relatoriogeracontagemsetorexcel.php?contagem='+contagem+'&invdata='+invdata, 'Report', 'status=yes, scrollbars=yes, location=no, toolbar=no, directories=no, resizable=no, fullscreen=yes');
		}
		else {
			window.open('relatoriogeracontagemexcel.php?contagem='+contagem+'&invdata='+invdata, 'Report', 'status=yes, scrollbars=yes, location=no, toolbar=no, directories=no, resizable=no, fullscreen=yes');
		}	
	}	
}

...
function load_grid()
{
	...
	window.open('relatoriogerasobraantigoinventario.php?estoque='+estoque+grupo+subgrupo+'&nome='+nome+'&invdata='+invdata, 'Report', 'status=yes, scrollbars=yes, location=no, toolbar=no, directories=no, resizable=no, fullscreen=yes');
}
...
function imprime_estoque(request)
{
	var xmldoc=request.responseXML;
  	...
  	var tabelax="<span class=\"custom-dropdown\"><span class=\"custom-dropdown\"><select id=\"estoque\" name=\"estoque\" size=\"1\"><option value=\"0\">-- Escolha um Estoque --</option>";

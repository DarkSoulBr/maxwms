...
var gravar_est_ini = 0;
var gravar_est_fim = 0;
var gravar_cid_ini = 0;
var gravar_cid_fim = 0;
var gravar_cid_rem = 0;
var gravar_cid_des = 0;
...
function cancelarPedido()
{
	...
	window.open('incluirctenovo.php', "_self" );
...
function processXMLparametros(obj)
{
	...
	dadospesquisaparametros2(1);
}	

function dadospesquisaparametros2(valor)
{	
	...
	{	  	
		...
		ajax2.open("POST", "cteparametrospesquisa.php", true);
		ajax2.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
		...
		{			
            if(ajax2.readyState == 4 )
            {
				if(ajax2.responseXML)
				{					
			    	processXMLparametros2(ajax2.responseXML);
			   	}
				else {
		...
		var params = "parametro="+valor+'&parametro2='+vusuario;
        ajax2.send(params);
...
function processXMLparametros2(obj)
{
	...
	{		
        ...
        for(var i = 0 ; i < dataArray.length ; i++)
        {
			...
			document.getElementById('gerrntrc').value = descricao5;			
				
		}
	}		
	
	new ajax ('infoseguroseleciona.php?usucodigo='+ vusuario
        , {onComplete: imprime_serv_ini});	
	
}

function imprime_serv_ini(request){
...
function itemcomboestadofim(valor){
	...
	dadospesquisacidadefim(valor,0);
}
...
function dadospesquisacidade(valor,valor2) {
   ...
   function dadospesquisacidadefim(valor,valor2) {
...
function dadospesquisaibgereprem(valor)
{
	...
	if(ajax4)
	{
        ...
        ajax4.open("POST", "cidadesibge.php", true);
		...
		ajax4.onreadystatechange = function()
		{
            ...
            if(ajax4.readyState == 4 )
            {
			   ...
			   {
			      processXMLibgereprem(ajax4.responseXML);
			   }
			   else
			   {
			       // caso não seja um arquivo XML emite a mensagem abaixo
...
function dadospesquisatipoexpedidor(valor) {
   ...
   function processXMLtipoexpedidor(obj){
	...
	document.forms[0].expedidor.options.length = 0;
	  ...
	  //Posiciona na opcao sem Expedidor
	  itemcomboexpserv(2); //Sem Expedidor
	  dadospesquisaverexpedidor(2); //Desabilita os Campos
...
function dadospesquisatiporecebedor(valor) {
   ...
   function processXMLtiporecebedor(obj){
	...
	document.forms[0].recebedor.options.length = 0;
	  ...
	  itemcomborecserv(2); //Sem Recebedor
	  dadospesquisaverrecebedor(2); //Desabilita os Campos
...
function dadospesquisaibgereprec(valor)
{
	...
	if(ajax5)
	{
        ...
        ajax5.open("POST", "cidadesibge.php", true);
		...
		ajax5.onreadystatechange = function()
		{
            ...
            if(ajax5.readyState == 4 )
            {
			   ...
			   {
			      processXMLibgereprec(ajax5.responseXML);
			   }
			   else
			   {
			       // caso não seja um arquivo XML emite a mensagem abaixo
...
function dadospesquisaibgerepdes(valor)
{
	...
	if(ajax3)
	{
        ...
        ajax3.open("POST", "cidadesibge.php", true);
		...
		ajax3.onreadystatechange = function()
		{
            ...
            if(ajax3.readyState == 4 )
            {
			   ...
			   {
			      processXMLibgerepdes(ajax3.responseXML);
			   }
			   else
			   {
			       // caso não seja um arquivo XML emite a mensagem abaixo
...
function importarnfe()
{
	...
	$("tab1").checked = false;	
	novaJanelaNfeCompleto("importanfe.php?usucodigo="+vusuario,"UPLOAD DE XML DE NOTA FISCAL ELETRONICA");
		
}


function novaJanelaNfeCompleto(href,t)
{
...
function  callbacknfecompleto()
{
	carreganfecompletoOK();
}

function carreganfecompletoOK()
{
	$("tab1").checked = true;	
	...
	new ajax('processanfecompleto.php?usuario='+vusuario , {onComplete: encerranfe});
	//window.open('processanfecompleto.php?usuario='+vusuario, '_blank');
	
}


function encerranfe(request)
{
	...
	if(xmldoc==null){
		...
		$a.unblockUI();	
		alert('Problema com a importação, favor enviar o XML que está tentando importar para o T.I.!');
		...
		var cteremrazao = itens[0].firstChild.data;
		...
		if(cteremrazao=='!ERRO!'){
			...
			var cteremendereco = itens[1].firstChild.data;
			var cterembairro = itens[2].firstChild.data;
			var cteremnumero = itens[3].firstChild.data;
			var cidrem = itens[4].firstChild.data;
			var cteremfone = itens[5].firstChild.data; 
			var cterempais = itens[6].firstChild.data; 
			var cteremuf = itens[7].firstChild.data; 
			var cterempessoa = itens[8].firstChild.data; 
			var cteremcep = itens[9].firstChild.data; 
			var cteremcnpj = itens[10].firstChild.data; 
			var cteremie = itens[11].firstChild.data; 
			var cteremcpf = itens[12].firstChild.data; 
			var remcodigo = itens[13].firstChild.data; 
			var cteremrg = itens[14].firstChild.data; 
			var cteremnguerra = itens[15].firstChild.data; 
			var cteremcomplemento = itens[16].firstChild.data; 
			var remcidcodigo = itens[17].firstChild.data; 
			var remcidcodigoibge = itens[18].firstChild.data;	
			var ctedesrazao = itens[19].firstChild.data; 	
			var ctedesendereco = itens[20].firstChild.data; 
			var ctedesbairro = itens[21].firstChild.data;
			var ctedesnumero = itens[22].firstChild.data;
			var ciddes = itens[23].firstChild.data;
			var ctedesfone = itens[24].firstChild.data; 
			var ctedespais = itens[25].firstChild.data; 
			var ctedesuf = itens[26].firstChild.data;		
			var ctedespessoa = itens[27].firstChild.data;
			var ctedescep = itens[28].firstChild.data; 
			var ctedescnpj = itens[29].firstChild.data; 
			var ctedesie = itens[30].firstChild.data; 
			var ctedescpf = itens[31].firstChild.data; 
			var descodigo = itens[32].firstChild.data; 
			var ctedesrg = itens[33].firstChild.data; 
			var ctedesnguerra = itens[34].firstChild.data; 
			var ctedescomplemento = itens[35].firstChild.data; 
			var descidcodigo = itens[36].firstChild.data; 
			var descidcodigoibge = itens[37].firstChild.data; 
			var ctedessuframa = itens[38].firstChild.data;
			var cterecrazao = itens[39].firstChild.data; 
			var cterecendereco = itens[40].firstChild.data; 
			var cterecbairro = itens[41].firstChild.data; 
			var cterecnumero = itens[42].firstChild.data; 
			var cidrec = itens[43].firstChild.data; 
			var cterecfone = itens[44].firstChild.data; 
			var cterecpais = itens[45].firstChild.data; 
			var cterecuf = itens[46].firstChild.data;		
			var cterecpessoa = itens[47].firstChild.data; 
			var ctereccep = itens[48].firstChild.data; 
			var ctereccnpj = itens[49].firstChild.data; 
			var cterecie = itens[50].firstChild.data;
			var ctereccpf = itens[51].firstChild.data;
			var reccodigo = itens[52].firstChild.data; 
			var cterecrg = itens[53].firstChild.data; 
			var cterecnguerra = itens[54].firstChild.data; 
			var ctereccomplemento = itens[55].firstChild.data; 
			var reccidcodigo = itens[56].firstChild.data; 
			var reccidcodigoibge = itens[57].firstChild.data;
			
			var cteufinicio = itens[58].firstChild.data;
			var cteuffinal = itens[59].firstChild.data;
			var ctecidinicio = itens[60].firstChild.data;	
			var ctecidfinal = itens[61].firstChild.data;
			var ctecfop = itens[62].firstChild.data;
			
			var ctecargaval = itens[63].firstChild.data; 
			var ctepropredominante = itens[64].firstChild.data; 
			var gerrntrc = itens[65].firstChild.data; 
			
			var ctereceber = itens[66].firstChild.data; 
			var ctetributos = itens[67].firstChild.data; 
			var sitcodigo = itens[68].firstChild.data; 
			var cteperreducao = itens[69].firstChild.data; 
			var ctebasecalculo = itens[70].firstChild.data; 
			var ctepericms = itens[71].firstChild.data; 
			var ctevalicms = itens[72].firstChild.data; 
			var ctecredito = itens[73].firstChild.data; 
			
			var ctepreencheicms = itens[74].firstChild.data; 
			var ctebaseicms = itens[75].firstChild.data; 
			var cteperinterna = itens[76].firstChild.data; 
			var cteperinter = itens[77].firstChild.data; 
			var cteperpartilha = itens[78].firstChild.data; 
			var ctevalicmsini = itens[79].firstChild.data; 
			var ctevalicmsfim = itens[80].firstChild.data; 
			var cteperpobreza = itens[81].firstChild.data; 
			var ctevalpobreza = itens[82].firstChild.data;	
			var cteentrega = itens[83].firstChild.data;
			var cteindlocacao = itens[84].firstChild.data; 
			var tpgcodigo = itens[85].firstChild.data; 
			var cteobsgeral = itens[86].firstChild.data;
			...
			trimjs(cterecbairro);if (txt=='0'){txt='';};cterecbairro = txt;
			...
			trimjs(gerrntrc);if (txt=='0'){txt='';};gerrntrc = txt;	
			...
			trimjs(tpgcodigo);if (txt=='0'){txt='';};tpgcodigo = txt;	
			...
			itemcombotomserv(1); //Tomador é o Remetente
			...
			remufibge = remcidcodigoibge;	
			
			gravar_cid_rem = remcidcodigoibge;
						
			dadospesquisaibgereprem(cteremuf);
			...
			desufibge = descidcodigoibge;				
			
			gravar_cid_des = descidcodigoibge;
			
			dadospesquisaibgerepdes(ctedesuf);
			...
			recufibge = reccidcodigoibge;				
			//window.setTimeout("dadospesquisaibgereprec('"+cterecuf+"')", 1000);	
			dadospesquisaibgereprec(cterecuf);	
			...
			gravar_est_ini = cteufinicio;
			gravar_est_fim = cteuffinal;
			gravar_cid_ini = ctecidinicio;
			gravar_cid_fim = ctecidfinal;
									
			$("cfop").value=ctecfop;
			if(ctecfop=='6353'){
				$("natureza").value='PRESTACAO DE SERVICO DE TRANSPORTE FORA DO ESTADO';
			}
			else {
				$("natureza").value='PRESTACAO DE SERVICO DE TRANSPORTE';
			}
			//verificacfop(ctecfop);
			
			format_val2(ctecargaval);		
			$("valorcarga").value=grnum;	
			$("produtopre").value=ctepropredominante;	
			$("gerrntrc").value=gerrntrc;	
...
function imprime_notax(request){
	...
	new ajax ('infoseguroseleciona.php?usucodigo='+ vusuario
        , {onComplete: imprime_servx});	
}

function imprime_servx(request){
	...
	new ajax ('valorservicocarregatotal.php?parametro='+ vusuario
        , {onComplete: imprime_total_servicosx});
...
function imprime_total_servicosx(request){
...
function load_grid2x(status)
{
	...
	var estadoini = gravar_est_ini;
	var estadofim = gravar_est_fim;
	...
	var cidadeini = gravar_cid_ini;
	var cidadefim = gravar_cid_fim;
	...
	var remcidcodigoibge	= gravar_cid_rem;	
	...
	var descidcodigoibge	= gravar_cid_des;	
	...
	{		
		...
		ajax2.onreadystatechange = function()
		{
            ...
            {	
				//conf(id_pedido);				
				if(ajax2.responseXML) {
      			      processXMLincluix(ajax2.responseXML);					  
      			}
            }
    	}	
	}	

	var params = 'flag=InserirOrcamento&tabela=ctenfe&pedido='+pedido+	
	'&numero_correto='+numero_correto+
	'&modelo='+modelo+
	'&serie='+serie+
	'&numero='+numero+
...
function processXMLincluix(obj){
	...
	if(dataArray.length > 0) {
        ...
        for(var i = 0 ; i < dataArray.length ; i++) {
			...
			numero_correto =  item.getElementsByTagName("pvnumero")[0].firstChild.nodeValue;
			numero_cte =  item.getElementsByTagName("ctenumero")[0].firstChild.nodeValue;	
			globalizado =  item.getElementsByTagName("globalizado")[0].firstChild.nodeValue;	
		}
	}
	else {
	...
	if(numero_cte==''){
		...
		if(globalizado==1){
			alert("Importação de NF-e concluída com sucesso!\n" + "Atenção, foi gerado um CT-e Globalizado!");			
		}
		else {
			alert("Importação de NF-e concluída com sucesso!");
		}

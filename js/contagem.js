...
if($("acao").value=="inserir"){
	new ajax ('contageminsere.php?fpgnome=' + document.getElementById('fpgnome').value + "&opcao=" + opcao , {onComplete: incluido});
}
else if($("acao").value=="alterar"){
	new ajax ('contagemaltera.php?fpgnome=' + document.getElementById('fpgnome').value + "&fpgcodigo=" + document.getElementById('fpgcodigo').value+ "&opcao=" + opcao, {onComplete: alterado});
}
}
...
new ajax ('contagemapaga.php?fpgcodigo=' + fpgcodigo, {onComplete: apagado});
limpa_form();
}
...
function limpa_form(){
	...
	if (document.querySelector('#opcoes')){
        ...
        idOpcao.innerHTML = "Faça a Pesquisa da Contagem";
 	}
...
function dadospesquisa(valor) {
	  ...
	  if(ajax2) {
		...
		ajax2.open("POST", "contagempesquisa.php", true);
   ...
   function processXML(obj){
	...
	document.forms[0].listDados.options.length = 0;
         ...
         for(var i = 0 ; i < dataArray.length ; i++) {
			...
			var opcao =  item.getElementsByTagName("opcao")[0].firstChild.nodeValue;

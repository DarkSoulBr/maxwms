...
function enviar()
{
    ...
    catch(e)
    {
	  ...
	  if(ajax3)
	  {
			...
			{	
				ajax3.open("POST", "veiculoscteinsere.php", true);
				...
				ajax3.onreadystatechange = function()
				{
					if(ajax3.readyState == 4 )
					{
						...
						{
							processXMLinsere(ajax3.responseXML);
						}
						else
						{
				...
				var params = 'veicodint=' + document.getElementById('veicodint').value +
						  ...
						  '&veirg=' + document.getElementById('veirg').value +
				...
				ajax3.send(params);	
				//window.open('veiculosinsere.php?' + params , '_blank');
			...
			{			
				...
				//window.open ('veiculosaltera.php?veicodigo=' + document.getElementById('veicodigo').value +
				new ajax ('veiculosctealtera.php?veicodigo=' + document.getElementById('veicodigo').value +
						  '&veicodint=' + document.getElementById('veicodint').value +	
						  '&veirenavam=' + document.getElementById('veirenavam').value +
						  '&veiplaca=' + document.getElementById('veiplaca').value +
						  '&veitara=' + document.getElementById('veitara').value +
						  '&veicapacidadekg=' + document.getElementById('veicapacidadekg').value +
				...
				if ( document.getElementById("radio1").checked==true){
					novo.text  = document.getElementById('veicodint').value;
				}
				else if ( document.getElementById("radio2").checked==true){
					novo.text  = document.getElementById('veirenavam').value;
				}
				else
				{
...
function verificacnpjcpf()
{
  ...
  if(ajax2) {
     ...
     ajax2.open("POST", "clientecteverificacnpjcpf.php", true);
...
function limpa_form(tipo)
{
	...
	document.getElementById("veitipproprietario").disabled=true;
	document.getElementById("listestados2").disabled=true;
	$("veirazao").value = "";
        ...
        for(var i = 0 ; i < dataArray.length ; i++)
        {
			...
			//	$("veipropuf").value=descricao17;
				...
				{		
					...
					document.getElementById("veitipproprietario").disabled=false;
					document.getElementById("listestados2").disabled=false;		
...
function codigo_fornecedor(valor)
{
	...
	{
		// deixa apenas o elemento 1 no option, os outros são excluídos
		ajax2.open("POST", "clientectecodpesquisa.php", true);
		ajax2.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function apagar(codigo){
	new ajax ('veiculoscteapaga.php?codigo=' + codigo, {});
	limpa_form();
}

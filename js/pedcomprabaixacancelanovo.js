...
function processXML(obj) {
    ...
    if (dataArray.length > 0) {
        ...
        for (var i = 0; i < dataArray.length; i++) {
            ...
            var pedidocross = item.getElementsByTagName("pedidocross")[0].firstChild.nodeValue;
            ...
            if (pedidocross == 1) {
                ...
                if ($("usuario").value == 359) {

                    alert("Atenção, pedido Cross-FUN! Usuário MOR com permissão para Continuar!");

                } else {

                    alert("Não é permitido cancelar a baixa de um pedido Cross-FUN!");
...
function cancelar() {
    ...
    if (confirm("Confirma o cancelamento da nota: " + document.getElementById('nota').value + " !")) {
        cancelar2();
...
function cancelar2() {
    ...
    let titulo = "PROCESSANDO, AGUARDE...";
    let mensagem = "CANCELANDO A BAIXA DO PEDIDO!";
    ...
    new ajax('pedcomprabaixacancconf.php?' + campovet1
            + '' + campovet2
            + '&notentcodigo=' + document.getElementById('notentcodigo').value
...
function imprimebaixa() {
    ...
    new ajax('pedcomprabaixacancimp.php?'
...
function limpar() {
    window.open("pedcomprabaixacancelanovo.php?", "_self");
}
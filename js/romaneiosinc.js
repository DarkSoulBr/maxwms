...
for (var i = 0; i < 500; i++) {
    ...
    vet[i][2] = "";
    vet[i][3] = "";
    vet[i][4] = 0; //valor rateado
    vet[i][5] = 0; //valor do pedido
}
...
function verificapedido() {
    ...
    //valor3=document.getElementById('volumespedido').value;
...
function dadospesquisapedido(valor) {
    ...
    if (txt != '') {
        ...
        if ($a("#listaPesquisaTransp").val() == "0") {
            ...
            var tipoPesquisa = 0;
            if (document.getElementById("radioped0").checked == true) {
                tipoPesquisa = 1;
            } else {
                tipoPesquisa = 2;
            }
...
function imprime_pedido(request)
{
    ...
    if (xmldoc != null)
    {
        ...
        if (codigo == 0) {
            ...
            //Se for a pesquisa por Embarques já vai para o Grid
            if (document.getElementById("radioped1").checked == true) {
                verificaacesso();
            }
...
function load_gridpedido() {
    ...
    new ajax('romaneiosseleciona2.php?pesquisa='
...
function dadospesquisapalm(valor, tipo) {
    ...
    if (ajax2) {
        ...
        ajax2.open("POST", "romaneiospesquisacoletor.php", true);
        ajax2.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
...
function processXMLpalm(obj, tipo) {
    ...
    if (carrega == 0) {
        document.forms[0].listveiculos.focus();
        carrega = 1;
    }
    ;
...
function Insere() {
    ...
    new ajax('romaneiosinserir.php?romnumeroano=' + document.getElementById('pvnumero').value
...
function apagarped(codigo) {
    ...
    new ajax('romaneiosapaga1.php?romcodigo=' + codigo + '&usucodigo=' + usuario
...
function processXMLromaneiosseleciona(obj) {
    alert("processXMLromaneiosseleciona()");
    //pega a tag dado
    var dataArray = obj.getElementsByTagName("dado");
    ...
    if (dataArray.length > 0) {
        ...
        for (var i = 0; i < dataArray.length; i++) {
            ...
            if (descricao8 == 0) {
                ...
                alert("on processXMLExp requesting ver1");
                ver1();
    ...
    if (dataArray.length > 0) {
        ...
        for (var i = 0; i < dataArray.length; i++) {
            ...
            for (var i2 = 0; i2 < 500; i2++) {
                ...
                if (vet[i2][0] == valor) {
                    //alert("Pedido(s) já Incluido(s) serão ignorados!");				   
                    i2 = 500;
                    j = 1;
                }
...
function pesquisaconferidos() {
    ...
    if (erro == 0) {
        if (document.getElementById("radiocon0").checked == true) {
            importaconferidos();
        } else {
            importaexpedidos();
        }
...
function importaconferidos()
{

    var transp = $a("#listaPesquisaTransp").val();

    var dtinicio = document.getElementById('dtinicio').value;
    var dtfinal = document.getElementById('dtfinal').value;
    var usuario = document.getElementById('usuario').value;
    novaJanela("consultaconferidos.php?inicio=" + dtinicio + "&final=" + dtfinal + "&transp=" + transp + "&usuario=" + usuario + "&local=3", "PEDIDOS CONFERIDOS")
    //window.open("consultaconferidos.php?inicio="+dtinicio+"&final="+dtfinal+"&usuario="+usuario+"&local=5","_blank")	

}

function importaexpedidos()
{
    var transp = $a("#listaPesquisaTransp").val();
    var dtinicio = document.getElementById('dtinicio').value;
    var dtfinal = document.getElementById('dtfinal').value;
    var usuario = document.getElementById('usuario').value;
    var coletor = document.forms[0].palm.options[document.forms[0].palm.selectedIndex].value;
    novaJanela("consultaexpedidos.php?inicio=" + dtinicio + "&final=" + dtfinal + "&transp=" + transp + "&usuario=" + usuario + "&coletor=" + coletor, "PEDIDOS EXPEDIDOS COLETOR")

    //window.open("consultaexpedidos.php?inicio=" + dtinicio + "&final=" + dtfinal + "&usuario=" + usuario + "&coletor=" + coletor, "_blank")
...
function  callback()
{
    ...
    //Processa os Pedidos selecionados pelo Usuário na tela de pesquisa e alimenta o Grid
    importaescolhidos(usuario);
}

function importaescolhidos(valor) {
    ...
    if (ajax2) {
        ...
        ajax2.open("POST", "importapedidosescolhidos.php", true);
        ajax2.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
        ...
        ajax2.onreadystatechange = function () {
            ...
            if (ajax2.readyState == 4) {
                if (ajax2.responseXML) {
                    processXMLEsc(ajax2.responseXML);
                } else {
                }
            }
        }
...
function imprime_transportadora(request) {
    ...
    let tabela = '<select id="listaPesquisaTransp" name="listaPesquisaTransp">';

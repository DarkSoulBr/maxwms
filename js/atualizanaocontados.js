...
function load_grid_estoque4(){
      new ajax ('cadastroconsultaset.php', {onComplete: imprime_estoque4});
}
...
function imprime_estoque4(request)
{
	var xmldoc=request.responseXML;
  	...
  	var tabelax="<span class=\"custom-dropdown\"><select id=\"estoque4\" name=\"estoque4\" size=\"1\"><option value=\"0\">-- Escolha um Setor --</option>";
...
function imprelatorioexexcel(){
	...
	var coletor 	= document.getElementById("estoque2").value;
	var estoque 	= document.getElementById("estoque3").value;
	var setor 		= document.getElementById("estoque4").value;
	...
	if(invdata==''){
	...
	else if(coletor==0 || coletor==''){
		alert( 'Selecione o coletor !');
	}
	else if(setor==0 || setor==''){
		...
		//Processa 
		/*
		window.open('finalizageranaocontados.php?invdata='+invdata
				+ '&contagem=' + contagem 
				+ '&coletor=' + coletor  
				+ '&estoque=' + estoque  
				+ '&setor=' + setor   
		...
		*/		
		...
		new ajax('finalizageranaocontados.php?invdata='+invdata
				+ '&contagem=' + contagem 
				+ '&coletor=' + coletor  
				+ '&estoque=' + estoque  
				+ '&setor=' + setor   
...
function imprimesetorencerra(request)
{
	...
	tabela="Processamento Encerrado!";	
	alert( tabela );
	tabela="";	

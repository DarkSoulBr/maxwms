...
function load_grid()
{
	...
	var i = 0; var grupo = "&grupo="; var auximp = "";
	...
	var estoqueimpresso1  = document.getElementById('estoqueimpresso1').checked;
	var estoqueimpresso2	= document.getElementById('estoqueimpresso2').checked;
	
	if(estoqueimpresso1==true)
	auximp = 1;
	else if(estoqueimpresso2==true)
		 auximp = 2;
		 else auximp=1;

	window.open('relatoriogeracontinventario.php?estoque='+estoque+grupo+subgrupo+'&nome='+nome+'&invdata='+invdata+'&estoqueimpresso='+auximp, 'Report', 'status=yes, scrollbars=yes, location=no, toolbar=no, directories=no, resizable=no, fullscreen=yes');
}
...
function imprimirconsulta(){
	...
	if(datainventario.length<=9)
	{
		...
		if(estoque=="" || estoque=="0")
		{
			...
			if(estoqueimpresso1==false && estoqueimpresso2==false)
			{
				alert("Selecione um tipo de Custo!");
				erro = 1;
			}
			else

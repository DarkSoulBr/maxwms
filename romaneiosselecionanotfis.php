...
if ($row) {
    ...
    for ($i = 0; $i < $row; $i++) {
        ...
        $sqlx = "SELECT a.pvnumero,b.notnumero,c.notnumero as toynumero FROM romaneiospedidos a 
                    LEFT JOIN notagua as b on a.pvnumero = b.pvnumero			
                    LEFT JOIN notadep as c on a.pvnumero = c.pvnumero			
        ...
        if ($rowx) {
            ...
            for ($ix = 0; $ix < $rowx; $ix++) {
                ...
                $toy_nota = pg_fetch_result($sqlx, $ix, "toynumero");

                if ($ped_nota == '' && $toy_nota == '') {
                    if ($pedidos == '') {

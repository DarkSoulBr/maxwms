...
class banco{
	...
	function apaga($tabela,$tctcodigo){
		$this->query="DELETE FROM $tabela WHERE tctcodigo=$tctcodigo";
		pg_query($this->query) or die ("Nao foi possivel apagar o registro na base");
	}

	function altera($tabela,$alteracao,$tctcodigo){
		$this->query="UPDATE $tabela SET $alteracao WHERE tctcodigo=$tctcodigo";

...
class banco{
	...
	function apaga($tabela,$estcodigo){
		$this->query="DELETE FROM $tabela WHERE estcodigo=$estcodigo";
		pg_query($this->query) or die ("Nao foi possivel apagar o registro na base");		
	}
	
	function altera($tabela,$alteracao,$estcodigo){
		$this->query="UPDATE $tabela SET $alteracao WHERE estcodigo=$estcodigo";

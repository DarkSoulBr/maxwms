...
class banco{
	...
	function apaga($tabela,$cfocodigo){
		$this->query="DELETE FROM $tabela WHERE cfocodigo=$cfocodigo";
		pg_query($this->query) or die ("Nao foi possivel apagar o registro na base");
	}

	function altera($tabela,$alteracao,$cfocodigo){
		$this->query="UPDATE $tabela SET $alteracao WHERE cfocodigo=$cfocodigo";

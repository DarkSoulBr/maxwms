...
class banco{
	...
	function apaga($tabela,$tpgcodigo){
		$this->query="DELETE FROM $tabela WHERE tpgcodigo=$tpgcodigo";
		pg_query($this->query) or die ("Nao foi possivel apagar o registro na base");
	}

	function altera($tabela,$alteracao,$tpgcodigo){
		$this->query="UPDATE $tabela SET $alteracao WHERE tpgcodigo=$tpgcodigo";

...
class banco {
    ...
    function apaga($tabela, $setcodigo) {
        $this->query = "DELETE FROM $tabela WHERE setcodigo=$setcodigo";
        pg_query($this->query) or die("Nao foi possivel apagar o registro na base");
    }

    function altera($tabela, $alteracao, $setcodigo) {
        $this->query = "UPDATE $tabela SET $alteracao WHERE setcodigo=$setcodigo";

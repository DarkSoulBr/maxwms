...
class banco{
	...
	function apaga($tabela,$tfocodigo){
		$this->query="DELETE FROM $tabela WHERE tfocodigo=$tfocodigo";
		pg_query($this->query) or die ("Nao foi possivel apagar o registro na base");
	}

	function altera($tabela,$alteracao,$tfocodigo){
		$this->query="UPDATE $tabela SET $alteracao WHERE tfocodigo=$tfocodigo";

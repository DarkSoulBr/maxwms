...
$romnumero = $_GET["romnumero"];
$romdata = $_GET["romdata"];
$transportadora = $_GET["transp"];
$romlocal = $_GET["romlocal"];
$codtransp = $_GET["codtransp"];
$enviaEmail = $_GET["email"];
if ($enviaEmail == '') {
    $enviaEmail = 0;
}
    ...
    if ($notfisvers == '5') {
        ...
        foreach ($pedidos as $value) {
            ...
            if (pg_num_rows($con_aux)) {
                ...
                $sql_aux = "SELECT * FROM notafil where pvnumero = $value";
                $con_aux = pg_query($sql_aux);
                if (pg_num_rows($con_aux)) {
                    //$notnumero 	= pg_fetch_result($con_aux,"0","notnumero");
                    //$notemissao = pg_fetch_result($con_aux,"0","notemissao");
    ...
    if ($enviaEmail == 1) {
        ...
        if ($email == '') {
            ...
            if ($where != '') {
                ...
                //Vai fazer um Looping para Processar todos os XMLS 
                $query = "select a.notnumero from notafil a 		
						$where order by a.notemissao asc";
                ...
                for ($i = 0; $i < pg_num_rows($cad); $i++) {
                    ...
                    $nomexml = ESTABELECIMENTO_LOJ . '001' . str_pad($array->notnumero, 7, "0", STR_PAD_LEFT) . '.xml';

                    //$aquivoxml = "/home/delta/datasul_toymania/datasul_nfe_copia/" . $nomexml;
                    $aquivoxml = "/home/delta/datasul_nfe_copia_30/" . $nomexml;
